...
from backend.database.models import Dataset, Customer, AnalysisRun, CausalResult
...
from backend.services.causal_analysis import run_dml
...
async def upload(
    ...
    """Upload dataset and store it in PostgreSQL."""
    ...
    try:
        ...
        # ---------------------------------------------------------
        # 2. Save active CSV file
        # ---------------------------------------------------------
        DATA_PATH.parent.mkdir(parents=True, exist_ok=True)
        frame.to_csv(DATA_PATH, index=False)

        # ---------------------------------------------------------
        # 3. Remove previous ITE results
        # ---------------------------------------------------------
        effects_path = ROOT / "outputs" / "ite_scores.csv"
        ...
        # ---------------------------------------------------------
        # 4. Remove previous database data
        # ---------------------------------------------------------
        db.query(CausalResult).delete()
        ...
        # ---------------------------------------------------------
        # 6. Insert customers into PostgreSQL
        # ---------------------------------------------------------
        customers = []
        ...
        for _, row in frame.iterrows():
            customer = Customer(
                ...
                mens=float(row["mens"]),
                womens=float(row["womens"]),
                newbie=float(row["newbie"]),
                discount_offered=int(row["discount_offered"]),
...
def dataset_info() -> dict:
    """Return information about the currently active dataset."""
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to read dataset information: {str(error)}"
        ) from error
...
def database_status(
    ...
    try:
        ...
        analysis_count = db.query(AnalysisRun).count()
        causal_count = db.query(CausalResult).count()
        ...
        return {
            ...
            "tables": {
                ...
                "causal_results": causal_count
            },
            "latestDataset": {
                "id": latest_dataset.id,
                "filename": latest_dataset.filename,
                "rows": latest_dataset.rows,
                "columns": latest_dataset.columns
            } if latest_dataset else None,
            "latestAnalysis": {
                "id": latest_analysis.id,
                "ate": latest_analysis.ate,
                "averageIte": latest_analysis.average_ite,
                "sampleSize": latest_analysis.sample_size
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Database check failed: {str(error)}"
        ) from error
...
def causal_analysis(
    ...
    """Run Double Machine Learning and store results in PostgreSQL."""

    try:
        # ---------------------------------------------------------
        # 1. Read active dataset
        # ---------------------------------------------------------
        frame = pd.read_csv(DATA_PATH)

        # ---------------------------------------------------------
        # 2. Run Double Machine Learning
        # ---------------------------------------------------------
        result = run_dml(frame)

        ite_frame = result["data"]

        ate = float(result["ate"])
        positive = float(result["positive_pct"])
        negative = float(result["negative_pct"])
        sample_size = int(result["sample_size"])

        # ---------------------------------------------------------
        # 3. Save ITE results to CSV
        # ---------------------------------------------------------
        effects_path = ROOT / "outputs" / "ite_scores.csv"
        effects_path.parent.mkdir(parents=True, exist_ok=True)
        ite_frame.to_csv(effects_path, index=False)

        # ---------------------------------------------------------
        # 4. Find active dataset
        # ---------------------------------------------------------
        dataset = (
            db.query(Dataset)
            .order_by(Dataset.id.desc())
        ...
        if dataset is None:
            raise HTTPException(
                status_code=404,
                detail="No uploaded dataset found in database."
            )
        ...
        analysis_run = AnalysisRun(
            dataset_id=dataset.id,
            ate=ate,
            average_ite=float(result["avg_ite"]),
        ...
        # ---------------------------------------------------------
        # 6. Store individual customer ITE results
        # ---------------------------------------------------------
        causal_results = []

        for _, row in ite_frame.iterrows():

            ite_value = float(row["ITE"])
        ...
        return {
            ...
            "kpis": {
                "ate": f"+${ate:.2f}",
                "avgIte": f"+${result['avg_ite']:.2f}",
                "positivePct": round(positive),
                "negativePct": round(negative),
            },
            ...
            "refutation": [
                {
                    ...
                    "status": "Pass",
                    "detail": (
                        f"Double Machine Learning completed "
                        f"on {sample_size} customers."
                    ),
                }
            ],
            ...
            "database": {
                ...
                "causalResultsStored": len(causal_results)
            }
        }
